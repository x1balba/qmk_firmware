...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_BASE] = LAYOUT_reviung41(
    ...
    GAMEPAD,    KC_Z,     KC_X,     KC_C,     KC_V,      KC_B,               KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,  KC_ENT,
                                    LCTL_T(KC_ESC), LALT_T(KC_BSPC), LSFT_T(KC_SPC),   RAISE,     LOWER
  ),
...
/* Light all LEDs cyan for base layer */
const rgblight_segment_t PROGMEM my_base_layer[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 11, HSV_BASE}
);
/* Light LED 11 purple when lower layer is active */
...
const rgblight_segment_t PROGMEM my_gamepad_layer[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 11, HSV_GAMEPAD}
);
...
const rgblight_segment_t* const PROGMEM my_rgb_layers[] = RGBLIGHT_LAYERS_LIST(
    my_base_layer,
    my_lower_layer,    // Overrides base layer
    my_raise_layer,    // Overrides other layers
    my_gamepad_layer   // Overrides other layers
);
